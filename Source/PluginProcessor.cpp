...
juce::AudioProcessorValueTreeState::ParameterLayout SimpleFlangerAudioProcessor::getParamLayout()
{
	...
	layout.add(std::make_unique<juce::AudioParameterFloat>(
		...
		PARAM_BEGIN_DELAY,
		juce::NormalisableRange<float>(0.f, MonoDelayLine::kMaxDelayTime),
		0.f,
		"ms"
	));
	...
	layout.add(std::make_unique<juce::AudioParameterFloat>(
		...
		PARAM_NOISEGEN,
		juce::NormalisableRange<float>(0.f, 512.f,1.f,0.3f),
		64.f
	));
	...
	layout.add(std::make_unique<juce::AudioParameterFloat>(
		SF_ID(PARAM_DAMP),
		PARAM_DAMP,
		juce::NormalisableRange<float>(0.f, 20000.f, 1.f, 0.3f),
		20000.f,
		"hz"
	));
...
void SimpleFlangerAudioProcessor::parameterValueChanged(int parameterIndex, float newValue)
{
	...
	if (parameterIndex == paramBeginDelay->getParameterIndex()) {
		...
		s.m_Supportor->setBeginDelay(val);
	}
